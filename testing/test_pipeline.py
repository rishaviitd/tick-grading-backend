...
This script orchestrates the complete pipeline from question JSON to final marking scheme:
1. Generate metadata from question JSON and images
2. Update syllabus.json with new concepts
3. Generate detailed solution and save as .md file
4. Generate comprehensive marking scheme using solution and CIE glossary
5. Organize all outputs in a structured directory

Usage:
    python test_pipeline.py question.json [--images image1.jpg image2.jpg] [--output-dir results]

Example:
    python test_pipeline.py sample_question.json --images diagram.jpg --output-dir test_results --log-level DEBUG
...
def run_solution_generation(
    ...
    try:
        ...
        solution = soln_gen.generate_solution(
            json_input=question_data,
            metadata=metadata,
            stream=False,
            image_paths=image_paths
        ...
        if solution:
            ...
            with open(solution_file, 'w', encoding='utf-8') as f:
                ...
                f.write(f"**Marks:** {question_data.get('marks', 'N/A')}\n\n")
...
def run_marking_scheme_generation(
    ...
    try:
        ...
        marking_scheme = markingscheme.generate_marking_scheme_text(
            json_input=question_data,
            metadata=metadata,
            stream=False,
            image_paths=image_paths,
...
def copy_source_files(
    ...
    try:
        ...
        original_dest_path = source_dir / Path(original_question_file).name
        ...
        for config in configs:
            ...
            if not is_internal_choice and total_marks is not None:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for config in configs:
                ...
                metadata_file = run_metadata_generation(
                ...
                if question_type not in ["MCQ", "Assertion-Reason"]:
                    step_start = time.time()
                    solution_file = run_solution_generation(
                ...
                step_start = time.time()
                marking_scheme_json = run_marking_scheme_generation(
                    question_file=question_file_old_format, metadata_file=metadata_file,
...
def main():
    ...
    print("Complete Pipeline Test Script")
    print("=" * 60)
    
    try:
        parser = argparse.ArgumentParser(
            description='Run complete pipeline for a single question.',
            formatter_class=argparse.RawDescriptionHelpFormatter,
            epilog="""
Examples:
  # Run pipeline for a single question
  python test_pipeline.py Q2.json --target-class 12
"""
        )
        
        parser.add_argument('input_file', help='Input JSON file (single question)')
        ...
        parser.add_argument('--no-syllabus-update', action='store_true',
                           help='Skip syllabus.json update step')
        parser.add_argument('--target-class', default=None, help='Target class for the question')
        ...
        logging.info(f"Processing single question: {args.input_file}")
        
        with open(args.input_file, 'r', encoding='utf-8') as f:
            question_data = json.load(f)
        question_id = question_data.get("question_identifier")
        if not question_id:
            question_id = question_data.get("ques_identifier", "unknown")

        output_path = create_output_directory(args.output_dir, question_id)
